...
export function validateUploadForm(formState: FormState): FormErrors {
  ...
  if (formState.examDate == undefined) newErrors.examDate = "Date is required";
    ...
    if (formState.examDate > new Date()) {
      console.log({ examDate: formState.examDate, iso: formState.examDate.toISOString(), date: new Date() })
      newErrors.examDate = "Date shouldn't be in the future."
    };
  }
